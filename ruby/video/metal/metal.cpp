...
struct VideoMetal : VideoDriver, Metal {
  ...
  auto size(uint32_t& width, uint32_t& height) -> void override {
  ...
  auto acquire(uint32_t*& data, uint32_t& pitch, uint32_t width, uint32_t height) -> bool override {
    if (sourceWidth != width || sourceHeight != height) {
      ...
      bytesPerRow = sourceWidth * sizeof(uint32_t);
    ...
    pitch = sourceWidth * sizeof(uint32_t);
    return data = buffer;
  }

  auto release() -> void override {}
  
  auto resizeOutputBuffers(uint32_t width, uint32_t height) {
  ...
  auto output(uint32_t width, uint32_t height) -> void override {
...
private:
  auto outputHelper(uint32_t width, uint32_t height, id<MTLTexture> sourceTexture) -> void {
